...
/* ────── Ambassador Full YOY Data (moved from Referrer tab) ────── */
interface AmbYOYEntry { 2023: number; 2024: number; 2025: number; 2026: number; type: string; _pace2026?: number }

const AMB_YOY_DATA: Record<string, AmbYOYEntry> = {
...
const YOY_AMB_TOTALS = { 2023: 1342 + 465, 2024: 8383 + 435, 2025: 5576 + 554, 2026: 1753 + 271 };
const YOY_AMB_COUNTS = { 2023: 6 + 24, 2024: 60 + 85, 2025: 82 + 127, 2026: 66 + 107 };
const YOY_INF_TOTALS = { 2023: 1342, 2024: 8383, 2025: 5576, 2026: 1753 };
const YOY_AIR_TOTALS = { 2023: 465, 2024: 435, 2025: 554, 2026: 271 };
...
export default function AmbassadorGrowth() {
  ...
  /* ── Full YOY ambassador table (moved from Referrer tab) ── */
  const sortedYOYNames = useMemo(() => {
    const names = Object.keys(AMB_YOY_DATA);
    names.forEach((n) => {
      AMB_YOY_DATA[n]._pace2026 = Math.round((AMB_YOY_DATA[n][2026] || 0) * ANN);
    });
  ...
  const yoyChartTop12 = useMemo(() => {
    return [...sortedYOYNames].sort((a, b) => (AMB_YOY_DATA[b]._pace2026 || 0) - (AMB_YOY_DATA[a]._pace2026 || 0)).slice(0, 12);
  }, [sortedYOYNames]);
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '2rem' }}>
      ...
      {/* ════════ Individual Ambassador Tracking (YOY) ════════ */}
      <div>
        <h3 style={sectionHeader}>Individual Ambassador Tracking</h3>
        <p style={sectionSub}>Year-over-year submissions. Influencers + Airway Ambassadors combined. 2026 = YTD through May 28.</p>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 12, marginBottom: 24 }}>
          {([2023, 2024, 2025, 2026] as const).map((y) => {
            ...
            let yoyColor = '';
            if (y === 2026) {
              const annualized = Math.round(YOY_AMB_TOTALS[2026] * ANN);
              yoyLine = `~${annualized.toLocaleString()} annualized pace`;
              yoyColor = '#888';
            } else if (y > 2023) {
              const prev = YOY_AMB_TOTALS[(y - 1) as 2023 | 2024 | 2025];
              const cur = YOY_AMB_TOTALS[y];
            ...
            return (
              <div key={y} style={{ background: '#fff', borderRadius: 8, border: '1px solid #e5e7eb', padding: 12 }}>
                ...
                <div style={{ fontSize: 22, fontWeight: 700, color: TP.navy, marginTop: 4 }}>{YOY_AMB_TOTALS[y].toLocaleString()}</div>
                <div style={{ fontSize: 11, color: '#888', marginTop: 2 }}>{YOY_AMB_COUNTS[y]} active ambassadors</div>
        ...
        <div style={{ height: 460 }}>
          <Bar
            data={{
              labels: yoyChartTop12.map((n) => n.length > 22 ? n.substring(0, 20) + '...' : n),
              datasets: [
                {
                  label: '2025 Actual',
                  data: yoyChartTop12.map((n) => AMB_YOY_DATA[n][2025] || 0),
                ...
                },
                {
                  ...
                  data: yoyChartTop12.map((n) => AMB_YOY_DATA[n]._pace2026 || 0),
                ...
                tooltip: {
                  callbacks: {
                    afterBody: (ctx: any) => {
                      ...
                      const name = yoyChartTop12[idx];
                      return `YTD actual: ${AMB_YOY_DATA[name][2026] || 0}`;
                    },
                  },
                },
              },
        ...
        <div style={{ overflowX: 'auto', marginTop: 24 }}>
          <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '0.84em', background: '#fff', borderRadius: 10, overflow: 'hidden' }}>
            ...
            <tbody>
                ...
                return (
                  <tr key={name} style={{ background: idx % 2 === 0 ? '#f9f9f9' : '#fff' }}>
                    ...
                    <td style={{ padding: '6px 8px', textAlign: 'center' }}>
                      <span style={{ color: d.type === 'Influencer' ? TP.teal : '#5BA88C', fontSize: '0.8em' }}>{d.type}</span>
                    </td>
                    {([2023, 2024, 2025, 2026] as const).map((y) => (
              ...
              {/* Subtotal rows */}
              {([
                { label: 'Influencer Subtotal', bg: '#3a5a7a', totals: YOY_INF_TOTALS },
                { label: 'Ambassador Subtotal', bg: '#4a6a8a', totals: YOY_AIR_TOTALS },
                { label: 'All Ambassadors (Program)', bg: TP.navy, totals: YOY_AMB_TOTALS },
              ] as const).map((row) => {
                const pace = Math.round(row.totals[2026] * ANN);
                const prev = row.totals[2025];
                const pctChg = ((pace - prev) / prev * 100).toFixed(0);
                const arrow = pace >= prev ? '↑ +' : '↓ ';
                return (
                  <tr key={row.label} style={{ background: row.bg, fontWeight: 'bold' }}>
                    <td style={{ padding: '8px 12px', color: '#fff' }}>{row.label}</td>
                    <td style={{ padding: '8px 8px', textAlign: 'center', color: '#fff', fontSize: '0.8em' }}>--</td>
                    {([2023, 2024, 2025, 2026] as const).map((y) => (
                      <td key={y} style={{ padding: '8px 10px', textAlign: 'right', color: '#fff' }}>{row.totals[y].toLocaleString()}</td>
                    ))}
